...
2026-09-15 新增部署代码后台检查（代码已实现，本次未重启）：启用提前通道时在同进程首次验证
固定 race 包装器，之后每轮结束后隔 30 秒检查。交易不再逐笔查部署代码或检查其 3 秒 TTL；
首次未通过不开放该路径，后续请求失败只记日志，确认代码变化则停用该路径直到人工复核/重启。
Feed 6 秒及其他动态证据/资金/报价门禁不变。细节和风险见
[最新交接](HANDOFF.md#2026-09-15-部署代码后台检查已实现未重启)。下文运行状态为历史记录，
不能依据旧 PID 或“未启动”的描述操作当前进程。

2026-09-14 实现交接。2026-09-15 已推送代码、停止旧进程、完成 008–010 和六关系 Kyber-only
配置准备，但保持急停、风险待确认、trial 未初始化，未启动实盘。
**最新实际状态和剩余人工步骤见 [停机准备结果](EARLY_FEED_PRELAUNCH_2026-09-15.md)。**
下方“当前结论/生产只读核对”保留 09-14 准备前的历史时点，不代表新表现在仍不存在。
...
2. 从 Feed 时间和接收时间计算的 6 秒窗口（2026-09-15 用户批准调整）贯穿接收、识别、决策、准备、签名前复核和最终发送检查；晚到、
