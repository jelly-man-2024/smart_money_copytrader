...
2026-09-16 更新：执行链路三项优化已落地，包含 0x 单接口报价构建、单轮并行预检复用和 HTTPS 连接池。
详见 [实现与只读验证结果](EXECUTION_LATENCY_2026-09-16.md)；关系配置已切换，旧进程已停，
等待操作员启动。核销及试运行续期见 [交接记录](HANDOFF.md)，尚无新版本实盘效果统计。
下文 2026-09-14 的统计与多轮检查描述保留作历史基线，不代表新版本运行结果。
