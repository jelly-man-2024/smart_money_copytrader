...
准备。钱包、策略、额度、私钥、风险确认、授权和 monitor 启动由使用者在下一阶段亲自完成。
...
- 不创建 mainnet 风险确认文件，不发送 approve，不签名或广播交易；
- 不启动 monitor、常驻脚本、LaunchAgent 或其他服务；
- 不复制旧服务器的 .env、数据库、var、风险文件或运行状态；
...
以下全部不在本轮授权内：钱包/私钥写入、relationship 插入或启用、付费 RPC 配置、风险确认、
...
不要复制其他机器的 `.venv`、`.env`、`var/`、SQLite、Docker volume、数据库 dump、风险确认文件或
...
当前参考值是175项 unittest全部通过、13笔历史回放，所有信号保持 `copy_eligible=false`。后续提交
...
测试阶段不要创建主网风险开关，也不要连接私钥库。单元测试成功后再创建急停文件：
...
- 没有 monitor/sm-copy 常驻进程，没有 relationship、私钥或风险文件，没有链上交易。
- Git 暂存区为空；不提交、不推送。
...
- Git最终状态：
- 阻塞或架构差异：
- 下一步由使用者完成：钱包、relationship、额度、RPC、风险确认、单实例切换和启动
```
...
7. 配置完成后先做只读 relationship/status、余额、allowance、nonce、执行审计和有限监听；再由
   使用者针对精确 follower、relationship、config snapshot和测试窗口创建仓库外0600风险确认。
8. `mainnet-approve-usdg` 与 `monitor --enable-mainnet-live` 都是会产生真实资产风险的下一阶段操作，
   只有使用者再次明确确认后才能执行。
