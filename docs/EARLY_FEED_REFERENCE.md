...
2026-09-15 运行时更新：monitor/run 已有独立 trial 提前执行入口，以下“未接入”是早期离线阶段
记录；当前接线见 EARLY_FEED_LIVE_HANDOFF.md。启用提前通道后，固定 race 部署代码由
`DeploymentMonitor` 启动验证并在每轮结束后隔 30 秒复核，交易复用结果、不检查部署快照 3 秒
TTL。首次未验证或已检测代码变化时该路径不可用；后续请求失败仅日志提醒、保留上次成功结果，
无缓存最长有效期。其他动态快照 TTL 不变。下面快照表的 3 秒部署规则仍用于历史离线回放，
不会因为 JSON 中自称“缓存有效”而取消；只有受信运行时显式注入监控器才选择新策略。
完整边界与归因字段见 [后台检查交接](HANDOFF.md#2026-09-15-部署代码后台检查已实现未重启)。
...
- 仅 Feed，原始 fresh=true；离线回放默认仍使用原 3 秒规则，拒绝未来时间。
  2026-09-15 经用户批准，运行时提前通道统一使用 6 秒（源时间年龄、接收时间年龄均须满足），
  接收、识别、决策、构建、签名及最终发送复核共用该阈值；普通观察器仍默认 3 秒。
  此调整不放宽下表账户/部署/策略/持仓快照 TTL，也不改变 QuotePolicy。
