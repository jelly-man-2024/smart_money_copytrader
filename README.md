...
提前运行时的 Feed 意向有效期现为 7 秒，同时约束源时间戳年龄及本地接收年龄；所有执行复核
继续检查，超过即拒绝。普通观察器及历史离线回放默认仍为 3 秒；报价、账户/持仓快照、
滑点和试运行限额不变。启动日志 `early_feed_max_age_seconds` 可确认新进程实际加载的阈值。
提前通道的固定 race 包装合约代码改由同进程后台任务启动验证、每轮完成后隔 30 秒复核；
交易复用已验证结果，不逐笔查询部署代码，也不再受部署快照 3 秒 TTL 限制。
首次未验证不开放该路径；后续 RPC 检查失败只记日志并保留上次结果，不设缓存最长有效期。
确认代码不匹配则停用该提前路径（包括已识别但尚在执行检查的意向），需操作员复核/重启；
不自动接受新哈希，不影响其他提前路径及原严格证据门槛。候选/提案保留所用哈希、区块和原采集时间。
检查间隔和持续检查失败期间的变化可能暂未发现，这是该策略接受的风险；不是已证明合约永不可变。
详见 [部署检查交接](docs/HANDOFF.md#2026-09-15-部署代码后台检查已实现未重启)。
...
回执后的 Relay 自动入账补证会跳过严格匹配 calldata 与单条 Transfer 的顶层标准
transfer/transferFrom（其他日志仅允许同 token 标准 Approval），保留入账信号和跳过依据，不判为 BUY。
未知/复杂路径继续查询；无标记跨链直接交付也可能被过滤，这是接受的覆盖取舍。
该过滤不改变 Feed 提前买卖规则，详见[入账补证过滤](docs/copy_trade_flow.md#34-relay被动buy普通收币如何排除)。
...
聚合器模拟失败时，`live_execution_abandoned.simulation_failure` 保存有界的未签名 `eth_call`
参数、计划报价区块、模拟时点及脱敏回滚信息；覆盖准备、签名前和广播前复核三个阶段。
`pending` 状态未固定，报价区块不是模拟区块，不承诺事后精确还原同一链上状态。
它写入现有 JSONL 日志，不另建数据库表，不增加成功路径 RPC，也不改变交易门禁。
字段、大小限制及查询方法见 [诊断交接](docs/HANDOFF.md#2026-09-15-模拟失败现场诊断已实现本次改动未重启上线)。
已保存的 Kyber 失败调用可通过独立、显式选择的历史 callTracer 入口定位子调用；
使用限制及实例见 [历史调用追踪](docs/HANDOFF.md#2026-09-15-独立历史-calltracer-诊断与-v4-token-转出回滚定位)。
